...
shep_clustered <- data %>%
  ...
  mutate(
    # ==========================================
    r_clust_discrimination = pmax(as.numeric(r_DiscrimREC), as.numeric(r_DiscrimGend), as.numeric(r_DiscrimAge), as.numeric(r_DiscrimOther)),
    
    r_clust_isolation = pmax(as.numeric(r_FeelLonely), as.numeric(r_LackSocial)),
    
    r_clust_basics = pmax(as.numeric(r_PayBasics), as.numeric(r_LackFood), as.numeric(r_HaveHousing)),
    
    r_clust_work_edu = pmax(as.numeric(r_FindWork), as.numeric(r_LackEduc)),
    
    n_clust_basics = pmax(as.numeric(n_HelpBasics), as.numeric(n_HelpFood), as.numeric(n_HelpHousing)),
    
    n_clust_work_edu = pmax(as.numeric(n_HelpWork), as.numeric(n_HelpEduc)),
    
    n_clust_discrim_iso = pmax(as.numeric(n_HelpDiscrim), as.numeric(n_HelpIsolation), as.numeric(n_HelpLoneliness))
  ) %>%
  ungroup() %>%
  mutate(across(
