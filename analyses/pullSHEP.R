...
miss_grid_need<-outcomes.need%>%
  dplyr::select(SURVID ,miss_all_need)
...
miss_grid_risk<-outcomes.risk%>%
  dplyr::select(SURVID ,miss_all_risk)
...
miss_grid_sat<-outcomes.sat%>%
  dplyr::select(SURVID ,miss_all_sat)
...
miss_grid_health<-outcomes.health%>%
  dplyr::select(SURVID ,miss_all_health)
...
rm(list = ls()[!ls() %in% c("dat_fin", "risk_vars", "need_vars")])
